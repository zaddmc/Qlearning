...
class BackPropProgram {
    ...
    static double[][] ReadAllData(int numInput, int numOutput, out int numRows) {

        StreamReader sr = new(Program.GetFilePath());
        ...
        for (int i = 0; i < numRows; i++) {
            string[] fileArr = dataList[i].Split('|');
            string[] inputStr = fileArr[0].Split(',');
    ...
    public static void ShowMatrix(double[][] matrix, int numRows, int decimals, bool indices) {
        int len = matrix.Length.ToString().Length; // note to james, what a niche problem that i love, th esolution htat is
        for (int i = 0; i < numRows; ++i) {
            if (indices == true)
...
public class NeuralNetwork {
    ...
    public double[] ComputeOutputs(double[] xValues) {
        ...
        inputs = xValues; // my improvement to next code block
    ...
    public double[] Train(double[][] trainData, int maxEpochs, double learnRate, double momentum) {
        ...
        int errInterval = 100; // interval to check error
